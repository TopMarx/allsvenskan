...
Allsvenskan Fantasy fantasy game data, fetched directly from the official fantasy API at
`https://en.fantasy.allsvenskan.se/api/`.
...
.
├── .github/workflows/
│   └── allsvenskan-fetch.yml        # scheduled daily fetch (1am UTC, retries 2am/3am)
├── scripts/
...
│       ├── allsvenskan-bootstrap_{season}.json
│       ├── allsvenskan-fixtures_{season}.json
│       ├── allsvenskan-dream-team_{season}.json
│       ├── allsvenskan-regions_{season}.json
...
└── latest/                          # always the most recent data, season-agnostic
    ├── allsvenskan-bootstrap.json
    ├── allsvenskan-fixtures.json
...
| File | Description |
|---|---|
| `data/{season}/allsvenskan-bootstrap_{season}.json` | Players, teams, gameweeks, game settings |
| `data/{season}/allsvenskan-fixtures_{season}.json` | All season fixtures with scores |
...
| `data/{season}/allsvenskan-dream-team_{season}.json` | Season dream team (updated each GW) |
| `data/{season}/allsvenskan-set-piece-notes_{season}.json` | Set piece taker notes (if the league API provides them) |
| `data/{season}/allsvenskan-regions_{season}.json` | Region/nationality reference (if the league API provides it) |
...
| `data/{season}/csv/teams.csv` | All teams listed in the bootstrap (16 compete in Allsvenskan; the platform may list extra historical entries) |
...
| File | Description |
|---|---|
| `latest/allsvenskan-bootstrap.json` | Current bootstrap |
| `latest/allsvenskan-fixtures.json` | Current fixtures |
...
Seasons run within a single calendar year (spring–autumn) and are identified by that year. The 2026 season is `2026`.
...
# Latest bootstrap
url = "https://raw.githubusercontent.com/TopMarx/allsvenskan/main/latest/allsvenskan-bootstrap.json"
...
```
https://raw.githubusercontent.com/TopMarx/allsvenskan/main/data/{season}/allsvenskan-bootstrap_{season}.json
https://raw.githubusercontent.com/TopMarx/allsvenskan/main/latest/allsvenskan-bootstrap.json
```

### Clone the repo

```
git clone https://github.com/TopMarx/allsvenskan.git
```
